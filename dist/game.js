...
/* =========================================================================
   ...
   ========================================================================= */
/* ---------- TOPICS ---------- */
const TOPIC_KEYS = ["eastern", "shortage", "vale", "uprising", "escape"];
const RULE_TEXT = {
...
let buried = 0;
let released = 0;
let currentLetter; // always set by renderLetter before use
let currentTokens = [];
...
function tokenize(letter) {
    const tokens = [];
    letter.parts.forEach((part) => {
        const text = typeof part === "string" ? part : part.t;
        const topic = typeof part === "string" ? null : part.topic;
        text.trim().split(/\s+/).forEach((word) => {
            if (word)
                tokens.push({ word, topic, blacked: false });
        });
    });
    return tokens;
}
...
function startDay() {
    ...
    while (pool.length < LETTERS_PER_DAY)
        pool = pool.concat(shuffle(LETTERS.slice()));
...
function renderLetter() {
    ...
    const meters = '<div id="meters">' +
        ...
        "</div>";
    const rules = '<aside id="rulebook"><h2>Standing Orders</h2><ol id="rules">' +
        forbidden.map((t) => '<li class="' + (prevForbidden.includes(t) ? "" : "new") + '">' +
...
function handleTimeout() {
    ...
    byId("btn-file").disabled = true;
    byId("btn-clear").disabled = true;
...
function wireDesk() {
    ...
    doc.addEventListener("pointerdown", (e) => {
        if (!markerActive || locked)
            return;
        const target = e.target;
        const span = target ? target.closest(".tok") : null;
        if (!span)
            return;
        e.preventDefault();
...
function setTokEl(i, on) {
    const el = document.querySelector('.tok[data-i="' + i + '"]');
    if (el)
        el.classList.toggle("blacked", on);
}
/* ---------- THE VERDICT ---------- */
function fileDocument() {
    ...
    currentTokens.forEach((t, i) => {
        if (t.topic && forbidden.includes(t.topic))
            required.add(i);
        if (t.blacked)
            blacked.add(i);
    });
    let missed = 0;
    let excess = 0;
    required.forEach((i) => { if (!blacked.has(i))
        missed++; });
    blacked.forEach((i) => { if (!required.has(i))
        excess++; });
    const fb = byId("feedback");
    let stampText;
    let stampColor;
    ...
    if (required.size > 0 && blacked.size === 0) {
        ...
        if (currentLetter.conscience) {
            ...
            stampText = "FLAGGED";
            stampColor = "var(--stamp)";
            fbClass = "bad";
            mistake = true;
            const brokenTopic = currentTokens[[...required][0]].topic;
...
function renderInterstitial(flawless) {
    statusEl.textContent = "";
